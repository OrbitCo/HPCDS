...
{
    [Authorize]
    public class vInventoryByOrgsController : ApiController
    {
        public IHttpActionResult Get()
        {
            using (var context = new hpcdsDbContext())
            {
                IQueryable<vInventoryByOrg> list = from data in context.vInventoryByOrgs select data;
                List<vInventoryByOrg> values = list.ToList();
