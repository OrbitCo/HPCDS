...
{
    [Authorize]
    public class InventoryBNPMChemicalsController : ApiController
    {
        public IHttpActionResult Get()
        {
            using (var context = new hpcdsDbContext())
            {
                IQueryable<InventoryBNPMChemical> list = from data in context.InventoryBNPMChemicals select data;
                List<InventoryBNPMChemical> values = list.ToList();
