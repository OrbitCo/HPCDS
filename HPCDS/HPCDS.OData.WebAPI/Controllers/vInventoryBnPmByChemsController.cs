...
{
    [Authorize]
    public class vInventoryBnPmByChemsController : ApiController
    {
        public IHttpActionResult Get()
        {
            using (var context = new hpcdsDbContext())
            {
                IQueryable<vInventoryBnPmByChem> list = from data in context.vInventoryBnPmByChems select data;
                List<vInventoryBnPmByChem> values = list.ToList();
