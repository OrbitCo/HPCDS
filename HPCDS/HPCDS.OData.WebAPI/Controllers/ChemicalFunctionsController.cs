...
{
    [Authorize]
    public class ChemicalFunctionsController : ApiController
    {
        public IHttpActionResult Get()
        {
            using (var context = new hpcdsDbContext())
            {
                IQueryable<ChemicalFunction> list = from data in context.ChemicalFunctions select data;
                List<ChemicalFunction> values = list.ToList();
