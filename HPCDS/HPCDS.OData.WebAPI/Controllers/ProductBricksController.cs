...
{
    [Authorize]
    public class ProductBricksController : ApiController
    {
        public IHttpActionResult Get()
        {
            using (var context = new hpcdsDbContext())
            {
                IQueryable<ProductBrick> list = from data in context.ProductBricks select data;
                List<ProductBrick> values = list.ToList();
