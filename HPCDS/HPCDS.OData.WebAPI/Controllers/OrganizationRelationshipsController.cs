...
{
    [Authorize]
    public class OrganizationRelationshipsController : ApiController
    {
        public IHttpActionResult Get()
        {
            using (var context = new hpcdsDbContext())
            {
                IQueryable<OrganizationRelationship> list = from data in context.OrganizationRelationships select data;
                List<OrganizationRelationship> values = list.ToList();
