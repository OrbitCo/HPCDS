...
{
    [Authorize]
    public class vInventoryBnPmWithChemsController : ApiController
    {
        public IHttpActionResult Get()
        {
            using (var context = new hpcdsDbContext())
            {
                IQueryable<vInventoryBnPmWithChem> list = from data in context.vInventoryBnPmWithChems select data;
                List<vInventoryBnPmWithChem> values = list.ToList();
