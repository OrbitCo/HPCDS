...
{
    [Authorize]
    public class PublicViewableDateSettingsController : ApiController
    {
        public IHttpActionResult Get()
        {
            using (var context = new hpcdsDbContext())
            {
                IQueryable<PublicViewableDateSetting> list = from data in context.PublicViewableDateSettings select data;
                List<PublicViewableDateSetting> values = list.ToList();
